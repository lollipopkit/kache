...
Kache records the content hash of each large input file it reads, keyed by path, so an unchanged file is not hashed again. GC deletes hashes recorded more than 30 days ago, 500 per index write, so builds never wait long on the delete. One sweep spends at most about 5 seconds on it, and a larger backlog continues in the next sweep. A lookup does not refresh a hash, so a file read all month without changing is hashed again once.
